...
| `g` / `G`, `Home` / `End` | First / last visible node |
| `Ctrl-u` / `Ctrl-d`, `PageUp` / `PageDown` | Move up / down by a page |
| `Space` / `Enter` | Toggle the selected container |
| `e` / `c` | Expand / collapse the whole selected branch |
| `-` / `+` (or `=`) | Shrink / grow the tree pane |
...
Press `?` in the app for the complete keyboard and mouse reference, or `q` to quit without output.
